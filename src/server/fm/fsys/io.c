...
iNode load_inode(int index)
{
    iNode inode;
    inode.index = index;
    raw_read(INODE + index * sizeof(iNode_entry), &inode.entry, sizeof(iNode_entry), FSYSD);
    return inode;
}

void save_inode(iNode *inode)
{
    raw_write(INODE + inode->index * sizeof(iNode_entry), &inode->entry, sizeof(iNode_entry), FSYSD);
}
